...
INSERT INTO "users" ("id", "name", "email", "email_verified", "created_at", "updated_at") VALUES
	('seed-user-alice', 'Alice', 'alice@bookkeeping.local', true, now(), now()),
	('seed-user-bob', 'Bob', 'bob@bookkeeping.local', true, now(), now()),
	('seed-user-charlie', 'Charlie', 'charlie@bookkeeping.local', true, now(), now())
...
UPDATE "categories" SET "user_id" = 'seed-user-alice' WHERE "user_id" IS NULL;--> statement-breakpoint
UPDATE "expenses" SET "user_id" = 'seed-user-alice' WHERE "user_id" IS NULL;--> statement-breakpoint
...
CREATE UNIQUE INDEX "categories_user_id_slug_unique" ON "categories" USING btree ("user_id","slug");--> statement-breakpoint
